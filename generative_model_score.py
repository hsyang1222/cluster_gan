...
class GenerativeModelScore:
    ...
    def load_or_make(self, train_loader, inception_model_info_path='../inception_model_info/', device='cuda:0', run_test=False, force_make=False) : 
        real_images_info_file_name = self.trainloaderinfo_to_hashedname(train_loader)
        os.makedirs(inception_model_info_path, exist_ok=True)
        if os.path.exists(inception_model_info_path + real_images_info_file_name) and not force_make: 
            print("Using generated real image info.")
            print(train_loader.dataset)
            self.load_real_images_info(inception_model_info_path + real_images_info_file_name)
            ...
            for each_batch in tqdm.tqdm(train_loader, desc='insert real dataset') : 
                ...
                self.put_real(X_train_batch)
                if run_test : break

            #generate real images info
            self.lazy_forward(batch_size=64, device=device, real_forward=True, shuffle=False)
            
            self.calculate_real_image_statistics()
            #save real images info for next experiments
            self.save_real_images_info(inception_model_info_path + real_images_info_file_name)
